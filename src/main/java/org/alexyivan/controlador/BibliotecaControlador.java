...
public class BibliotecaControlador implements IBibliotecaControlador {
    ...
    public BibliotecaControlador(IBibliotecaRepo bibliotecaRepo, IUsuarioRepo usuarioRepo, IJuegoRepo juegoRepo,
                                 ICompraRepo compraRepo, ITransactionManager tm) {
    ...
    public List<BibliotecaDto> verBibliotecaPersonal(long id, OrdenBusquedaBibliotecaEnum busquedaBiblioteca) {
        ...
        var bibliotecaPersonal = tm.inTransaction(() -> {
            ...
            var bibliotecaUsuario = bibliotecaRepo.obtenerTodos().stream().filter(b ->
                    b.getIdUsuario() == id);
            ...
            if (busquedaBiblioteca.equals(OrdenBusquedaBibliotecaEnum.ALFABETICO)) {
                ...
                return bibliotecaUsuario.sorted(Comparator.comparing(b -> {
                    ...
                    return Mapper.mapBibliotecaEntidadADto(b, usuarioDto.orElse(null),
    ...
    public Optional<BibliotecaDto> anhadirJuego(BibliotecaForm bibliotecaForm) {
        ...
        var bibliotecaAnhadida = tm.inTransaction(() -> {
            List<ErrorDto> erroresTx = new ArrayList<>();
            ...
            if (!biblioteca.stream().filter(b -> b.getIdJuego() ==
                    bibliotecaForm.getJuegoId()).findFirst().isEmpty()
                    && biblioteca.stream().filter(b -> b.getIdJuego() ==
                    bibliotecaForm.getUsuarioId()).findFirst().isPresent()) {
                erroresTx.add(new ErrorDto("juego", ErrorType.DUPLICADO));
            }
    ...
    public Optional<BibliotecaDto> eliminarJuego(BibliotecaForm bibliotecaForm) {
        var bibliotecaEliminada = tm.inTransaction(() -> {
            ...
            if (usuario.isPresent() && biblioteca.stream().filter(b -> b.getIdUsuario() ==
                    usuario.get().getId()).findFirst().isEmpty()) {
                erroresTx.add(new ErrorDto("id", ErrorType.NO_ENCONTRADO));
            }

            if (usuario.isPresent() && juego.isPresent() && biblioteca.stream()
                    .filter(b -> b.getIdJuego() == juego.get().getId()).findFirst().isEmpty()
                    && biblioteca.stream().filter(b -> b.getIdUsuario() == usuario.get().getId())
                    .findFirst().isPresent()) {
            ...
            return Optional.ofNullable
                    (Mapper.mapBibliotecaEntidadADto(bibliotecaBuscada.orElse(null), usuarioDto, juegoDto));
        });
    ...
    public Optional<BibliotecaDto> actualizarTempoJuego(BibliotecaForm bibliotecaForm, float horas) {
        ...
        var bibliotecaActualizada = tm.inTransaction(() -> {
            List<ErrorDto> erroresTx = new ArrayList<>();

            var usuario = usuarioRepo.obtenerPorId(bibliotecaForm.getUsuarioId());
            var juego = juegoRepo.obtenerPorId(bibliotecaForm.getJuegoId());
            var biblioteca = bibliotecaRepo.obtenerTodos().stream().filter(b -> b.getIdUsuario()
                    == usuario.get().getId()
            ...
            var bibliotecaActualizadaTx = bibliotecaRepo.actualizar(biblioteca.get().getId(),
                    new BibliotecaForm(biblioteca.get().getIdUsuario(), biblioteca.get().getIdJuego(),
                            biblioteca.get().getFechaAdquisicion(), (biblioteca.get().getHorasJugadasTotal() + horas),
                            biblioteca.get().getUltimaFechaDeJuego(),
                            biblioteca.get().getEstadoInstalacion()));

            var usuarioDto = Mapper.mapUsuarioEntidadADto(usuario.orElse(null));
            var juegoDto = Mapper.mapJuegoEntidadADto(juego.orElse(null));

            return Optional.ofNullable(Mapper.mapBibliotecaEntidadADto(bibliotecaActualizadaTx.orElse(null),
                    usuarioDto, juegoDto));
        });
    ...
    public Optional<BibliotecaDto> consultarUltimaSesion(BibliotecaForm bibliotecaForm) {
        var bibliotecaConsultada = tm.inTransaction(() -> {
            List<ErrorDto> erroresTx = new ArrayList<>();
            ...
            var biblioteca = bibliotecaRepo.obtenerTodos().stream()
                    .filter(b -> b.getIdUsuario() == usuario.get().getId()
                            && b.getIdJuego() == juego.get().getId()).findFirst();
            ...
            return Optional.ofNullable
                    (Mapper.mapBibliotecaEntidadADto(biblioteca.orElse(null), usuarioDto, juegoDto));
        });

        return bibliotecaConsultada;
    }

    @Override
    public Optional<EstadisticasBibliotecaDto> estadisticasBiblioteca(BibliotecaForm bibliotecaForm) {

        var estadisticasObtenidas = tm.inTransaction(() -> {
            List<ErrorDto> erroresTx = new ArrayList<>();
            ...
            float valorTotalBiblioteca = comprasUsuario
                    .map(c -> c.getPrecioSinDescuento() -
                            c.getPrecioSinDescuento() * (c.getDescuentoAplicado() / DESCUENTO))
                    .reduce(0f, (a, b) -> a + b);
